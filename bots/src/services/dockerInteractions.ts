...
import {
  ...
  createEmptyDockerEmbed,
  createStopModal,
} from '../components/docker.js';

const parseActionCustomId = (customId: string) => {
...
const updateManageMessage = async (interaction: BaseInteraction, selectedId?: string) => {
...
export const handleDockerSelect = async (interaction: StringSelectMenuInteraction) => {
  ...
  try {
    ...
    await interaction.followUp({
      content: `Cannot load docker information: ${(error as Error).message}`,
      ephemeral: true,
    });
  }
};

export const handleDockerAction = async (interaction: ButtonInteraction) => {
  ...
  try {
    ...
    await interaction.followUp({
      content: `Action failed: ${(error as Error).message}`,
      ephemeral: true,
    });
  } finally {
...
export const handleDockerStopModal = async (interaction: ModalSubmitInteraction) => {
  ...
  try {
    ...
    await interaction.editReply({
      content: `Failed to stop docker: ${(error as Error).message}`,
    });
  } finally {
    await updateManageMessage(interaction, dockerId);
  }
