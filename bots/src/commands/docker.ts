...
const buildManageReply = async (): Promise<InteractionReplyOptions> => {
  ...
  if (subcommand === 'status') {
    ...
    try {
      ...
      await interaction.editReply({
        content: `Unable to fetch docker information: ${(error as Error).message}`,
      });
    }
    return;
  }
...
const data = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) =>
    sub.setName('manage').setDescription('Open the interactive Docker management view'),
  )
  .addSubcommand((sub) =>
    sub
      .setName('status')
  ...
  .addSubcommand((sub) =>
    sub
      ...
      .addStringOption((option) =>
        option.setName('name').setDescription('Docker name to target').setRequired(true),
      )
      .addStringOption((option) =>
        option
          .setName('input')
